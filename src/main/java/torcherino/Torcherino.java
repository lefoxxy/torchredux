...
import torcherino.network.Networking;
import torcherino.platform.NetworkUtilsImpl;
import torcherino.platform.PlatformUtilsImpl;
...
public final class Torcherino {
    ...
    public Torcherino(IEventBus eventBus) {
        ...
        eventBus.addListener(Networking::register);
        if (!PlatformUtilsImpl.getInstance().isDedicatedServer()) {
            eventBus.addListener(ClientHelper::registerParticles);
        }
    }
